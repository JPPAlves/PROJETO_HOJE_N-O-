...
var JotForm = {
    ...
    validationRegexes: {
        email: /^(?:[\a-zA-Z0-9!#$%&'*+\/=?^_`{|}~-]+(?:\.[\a-zA-Z0-9!#$%&'*+\/=?^_`{|}~-]+)*|"(?:[\s\x01-\x08\x0b\x0c\x0e-\x1f\x21\x23-\x5b\x5d-\x7f]|\\[\x01-\x09\x0b\x0c\x0e-\x7f])*")@(?:(?:[a-zA-Z0-9](?:[a-zA-Z0-9-]*[a-zA-Z0-9])?\.)+[a-zA-Z0-9](?:[a-zA-Z0-9-]*[a-zA-Z0-9]){1,}|\[(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?|[a-zA-Z0-9-]*[a-zA-Z0-9]:(?:[\x01-\x08\x0b\x0c\x0e-\x1f\x21-\x5a\x53-\x7f]|\\[\x01-\x09\x0b\x0c\x0e-\x7f])+)\])$/,
    ...
    */
    ...
    checkCalculation: function (calc) {
        ...
        switch (resultFieldType) {
            ...
            case "appointment":
                ...
                setTimeout(function () {
                    if (calc.resultFieldProp === 'startdate') {
                        JotForm.appointments[result].forceStartDate(parsedOutput);
                    }
    ...
    updateDonationAmount: function (amount) {
        if (!JotForm.donationSourceField // source field is missing
            ...
            || JotForm.donationField.up('ul.form-field-hidden')) { return; }
        // amount is specified
    ...
    */
        ...
        $$('.form-product-image-with-options').forEach(function (image, i) {
            image.observe('click', function () {
                if (isIframeEmbedFormPure()) {
                    onProductImageClicked(i, true);
                } else {
                    onProductImageClicked(i, false);
                }
            });
        });
    },
    ...
    highLightLines: function () {
        ...
        $$('.form-line').each(function (l, i) {
            l.select('input, select, textarea, div, table div, button, div div span[tabIndex]:not([tabIndex="-1"])').each(function (i) {
            ...
            try {
                var imask = new Inputmask({
                    ...
                    oncomplete: function (e) {
                        ...
                        var splittedDate = e.target.value.split("-");
    ...
    saveLogToPaymentEvents: function (gateway, title, message) {
        new Ajax.Jsonp(JotForm.server, {
            parameters: {
                action: 'saveStripeLog',
                formId: $$('form').first().readAttribute('id'),
    ...
    */
    ...
    errored: function (input, message) {
        ...
        insertEl.insert(new Element('div', {
            ...
            role: 'alert'
        }).insert('<img src="' + preLink + 'images/exclamation-octagon.png"> ').insert(
    ...
    disableSubmitForCard: function (action, isConditionMatch) {
        ...
        JotForm.disableSubmitButtonMessage = (action && action.disableSubmitErrorMessage) ? action.disableSubmitErrorMessage : 's';
        if (!isConditionMatch) {
    ...
    setFieldValidation: function (input) {
        ...
        input.validateInput = function (deep, dontShowMessage) { // dontShowMessage param will be passed and handled by cardForm
            ...
            if (!JotForm.isVisible(input) && !input.hasClassName('g-recaptcha-response')) {
                return true; // if it's hidden then user cannot fill  then don't validate
            }
                        ...
                        var checkValues = inputs.map(function (e) {
                            ...
                            if (innerVals.include('required') && JotForm.isVisible(e)) {
                                ...
                                if (e.value.empty() || e.value.strip() == 'Please Select' || (JotForm.getContainer(input).getAttribute('data-type') === "control_address" && e.value.strip(" ").empty())) {
                                    ...
                                    if (!dontShow) { // card layout -> silent validation
                                        e.addClassName('form-validation-error');
                                    }
                                    return true;
                                } else {
                                    if (JotForm.getContainer(e).hasClassName("form-datetime-validation-error")) {
                                        return JotForm.errored(input, 'Enter a valid date', dontShow);
                                    }
                                }
                            }
        ...
        if (input.type == 'checkbox' || input.type == 'radio') {
            ...
            if (!vals.include('requireEveryRow') && !vals.include('requireEveryCell')) {
                input.observe('change', function () {
                    input.validateInput();
                });
            }
    ...
    createManualPrefill: function () {
        ...
        $$('.form-line').each(function (line) {
            ...
            if (allowedQuestions.indexOf(type) > -1) {
                ...
                line.select("input, textarea").each(function (elem) {
                    if (elem.hasAttribute('readonly')) {
                        elem.removeAttribute('readonly');
                    }
                });

                line.select("select").each(function (elem) {
    ...
    adjustWorkflowFeatures: function () {
        try {
            if (typeof document.get === 'object') {
                ...
                if (isRequestMoreInfo && wfTaskID && targetForm) {
                    targetForm.insert(new Element('input', { type: 'hidden', name: 'wfTaskID' }).putValue(wfTaskID));
                    targetForm.insert(new Element('input', { type: 'hidden', name: 'wfTaskType' }).putValue("request-more-info"));
                }
            }
        } catch (e) {
...
function onProductImageClicked(index, isEmbeddedwithIframe) {
